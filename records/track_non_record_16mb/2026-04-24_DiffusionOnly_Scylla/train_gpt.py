...
class CausalSmearGate(nn.Module):
...
class FeedForward(nn.Module):
    def __init__(self, args: Hyperparameters):
        super().__init__()
        hidden_dim = int(args.model_dim * args.mlp_mult)
        self.fc1 = nn.Linear(args.model_dim, hidden_dim, bias=False)
        self.fc2 = nn.Linear(hidden_dim, args.model_dim, bias=False)
    def forward(self, x: Tensor) -> Tensor:
        x = F.leaky_relu(self.fc1(x), negative_slope=0.5).square()
        return self.fc2(x)
...
class DiffusionBlock(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        a = self.attn(self.attn_norm(x)) * self.attn_scale.to(dtype=x.dtype)[None, None, :]
        m = self.mlp(self.mlp_norm(x)) * self.mlp_scale.to(dtype=x.dtype)[None, None, :]
        return x + a + m
...
class CausalDiffusionLM(nn.Module):
    def __init__(self, args: Hyperparameters):
        ...
        self.noisy_emb = nn.Embedding(args.vocab_size, args.model_dim)
        self.time_emb = nn.Embedding(args.diffusion_steps + 1, args.model_dim)
        self.pos_emb = nn.Parameter(torch.zeros(args.max_seq_len, args.model_dim, dtype=torch.float32))
        nn.init.normal_(self.tok_emb.weight, mean=0.0, std=args.tied_embed_init_std)
        nn.init.normal_(self.noisy_emb.weight, mean=0.0, std=args.tied_embed_init_std)
        nn.init.normal_(self.time_emb.weight, mean=0.0, std=args.tied_embed_init_std)
        nn.init.normal_(self.pos_emb, mean=0.0, std=args.tied_embed_init_std)
        self.bigram = (
    ...
    def make_noisy_targets(self, input_ids: Tensor, target_ids: Tensor) -> tuple[Tensor, Tensor]:
        b, t = input_ids.shape
        device = input_ids.device
        steps = max(int(self.args.diffusion_steps), 1)
        step_ids = torch.randint(1, steps + 1, (b, t), device=device, dtype=torch.long)
        noisy = torch.full_like(target_ids, max(0, min(self.args.vocab_size - 1, self.args.mask_token_id)))
        random_mask = torch.rand((b, t), device=device) < max(0.0, min(1.0, self.args.noise_random_prob))
        prefix_mask = (
            (~random_mask)
            & (torch.rand((b, t), device=device) < max(0.0, min(1.0, self.args.noise_prefix_prob)))
        )
        if random_mask.any():
            random_tokens = torch.randint(0, self.args.vocab_size, (b, t), device=device, dtype=torch.long)
            noisy[random_mask] = random_tokens[random_mask]
        if prefix_mask.any():
            noisy[prefix_mask] = input_ids[prefix_mask]
        return noisy, step_ids
    def hidden(self, input_ids: Tensor, noisy_ids: Tensor, step_ids: Tensor) -> Tensor:
        if input_ids.size(1) > self.args.max_seq_len:
            raise ValueError(f"Sequence length {input_ids.size(1)} exceeds MAX_SEQ_LEN={self.args.max_seq_len}")
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
        n = self.noisy_emb(noisy_ids)
        step_ids = step_ids.clamp(0, self.args.diffusion_steps)
        h = self.input_proj(torch.cat([x, n], dim=-1))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        noisy_ids, step_ids = self.make_noisy_targets(input_ids, target_ids)
        x = self.hidden(input_ids, noisy_ids, step_ids)
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        noisy_ids = torch.full_like(input_ids, max(0, min(self.args.vocab_size - 1, self.args.mask_token_id)))
        step = max(0, min(self.args.diffusion_steps, self.args.eval_noise_step))
        step_ids = torch.full_like(input_ids, step)
        return self.logits_from_hidden(self.hidden(input_ids, noisy_ids, step_ids))
...
def build_optimizer(args: Hyperparameters, model: nn.Module):
    embed_params = [model.tok_emb.weight, model.noisy_emb.weight, model.time_emb.weight, model.pos_emb]
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for diffusion-only training")
    ...
    base_model = CausalDiffusionLM(args).to(device=device)
    ...
    eval_model = CausalDiffusionLM(args).to(device=device)
