...
class Hyperparameters:
    ...
    segment_adapter_enabled = bool(int(os.environ.get("SEGMENT_ADAPTER", "0")))
    segment_adapter_layers = os.environ.get("SEGMENT_ADAPTER_LAYERS", "8,9,10")
    segment_adapter_rank = int(os.environ.get("SEGMENT_ADAPTER_RANK", "16"))
    segment_adapter_kernel = int(os.environ.get("SEGMENT_ADAPTER_KERNEL", "64"))
    segment_adapter_gate_init = float(os.environ.get("SEGMENT_ADAPTER_GATE_INIT", "-3.0"))
    segment_adapter_boundary_signal = bool(int(os.environ.get("SEGMENT_ADAPTER_BOUNDARY_SIGNAL", "1")))
...
class SegmentStateAdapter(nn.Module):
    def __init__(
        self,
        model_dim: int,
        rank: int,
        ...
        super().__init__()
        if rank <= 0:
            raise ValueError(f"segment adapter rank must be positive, got {rank}")
        if kernel_size <= 0:
            raise ValueError(f"segment adapter kernel must be positive, got {kernel_size}")
        self.rank = rank
        self.kernel_size = kernel_size
        self.boundary_signal = boundary_signal
        self.down = CastedLinear(model_dim, rank, bias=False)
        self.up = CastedLinear(rank, model_dim, bias=False)
        self.kernel_logits = nn.Parameter(torch.zeros(kernel_size, dtype=torch.float32))
        self.boundary_embed = nn.Parameter(torch.zeros(rank, dtype=torch.float32))
        self.gate = nn.Parameter(torch.tensor(gate_init, dtype=torch.float32))
        nn.init.orthogonal_(self.down.weight, gain=1.0)
        nn.init.zeros_(self.up.weight)
    def forward(self, x: Tensor, boundary_mask: Tensor | None = None) -> Tensor:
        z = self.down(x)
        if self.boundary_signal and boundary_mask is not None:
            b = boundary_mask.to(dtype=z.dtype).unsqueeze(-1)
            z = z + b * self.boundary_embed.to(dtype=z.dtype)[None, None, :]
        kernel = torch.softmax(self.kernel_logits.float(), dim=0).to(dtype=z.dtype)
        weight = kernel.view(1, 1, self.kernel_size).expand(self.rank, 1, self.kernel_size).contiguous()
        h = F.conv1d(z.transpose(1, 2), weight, padding=self.kernel_size - 1, groups=self.rank)
        h = h[:, :, : z.size(1)].transpose(1, 2)
        return x + torch.sigmoid(self.gate).to(dtype=x.dtype) * self.up(h)
...
class GPT(nn.Module):
    def __init__(
        ...
        ve_layers: str = "9,10",
        neg_slope: float = 0.5,
        segment_adapter_enabled: bool = False,
        segment_adapter_layers: str = "8,9,10",
        segment_adapter_rank: int = 16,
        segment_adapter_kernel: int = 64,
        segment_adapter_gate_init: float = -3.0,
        segment_adapter_boundary_signal: bool = True,
        segment_boundary_lut: Tensor | None = None,
    ):
        super().__init__()
        ...
        self.segment_adapter_layer_indices = (
            [int(x) for x in segment_adapter_layers.split(",") if x.strip()]
            if segment_adapter_enabled else []
        )
        self.segment_adapters = nn.ModuleDict(
            {
                str(i): SegmentStateAdapter(
                    model_dim,
                    segment_adapter_rank,
                    segment_adapter_kernel,
                    segment_adapter_gate_init,
                    segment_adapter_boundary_signal,
                )
                for i in self.segment_adapter_layer_indices
            }
        )
        if segment_boundary_lut is not None:
            self.register_buffer("segment_boundary_lut", segment_boundary_lut.detach().clone().bool(), persistent=False)
        else:
            self.segment_boundary_lut = None
    ...
    def _segment_boundary_mask(self, input_ids: Tensor) -> Tensor | None:
        if self.segment_boundary_lut is None or not self.segment_adapters:
            return None
        return self.segment_boundary_lut[input_ids]
    def _apply_segment_adapter(self, layer_idx: int, x: Tensor, boundary_mask: Tensor | None) -> Tensor:
        key = str(layer_idx)
        if key not in self.segment_adapters:
            return x
        return self.segment_adapters[key](x, boundary_mask)
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        skips: list[Tensor] = []
        ve_cache: dict = {}
        boundary_mask = self._segment_boundary_mask(input_ids)
        for i in range(self.num_encoder_layers):
            ...
            x = self.blocks[i](x, x0,
                ...
                v_embed=ve)
            x = self._apply_segment_adapter(i, x, boundary_mask)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[bi](x, x0,
                ...
                v_embed=ve)
            x = self._apply_segment_adapter(bi, x, boundary_mask)
        x = self.final_norm(x)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        segment_adapter_enabled=args.segment_adapter_enabled,
        segment_adapter_layers=args.segment_adapter_layers,
        segment_adapter_rank=args.segment_adapter_rank,
        segment_adapter_kernel=args.segment_adapter_kernel,
        segment_adapter_gate_init=args.segment_adapter_gate_init,
        segment_adapter_boundary_signal=args.segment_adapter_boundary_signal,
        segment_boundary_lut=is_boundary_token_lut,
    ).to(device).bfloat16()
    ...
    if base_model.segment_adapters:
        for p in base_model.segment_adapters.parameters():
    ...
    log0(
        "segment_adapter:"
        f"enabled={int(args.segment_adapter_enabled)} "
        f"layers={base_model.segment_adapter_layer_indices} "
        f"rank={args.segment_adapter_rank} kernel={args.segment_adapter_kernel} "
        f"boundary_signal={int(args.segment_adapter_boundary_signal)}"
    )
