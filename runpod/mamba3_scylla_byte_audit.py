...
def compute_eval_bytes(tokens: torch.Tensor, base: torch.Tensor, leading: torch.Tensor, boundary: torch.Tensor, seq_len: int):
    ...
    total_bytes = int(token_bytes.sum().item())
    return {
        "tokens": int(tgt_ids.numel()),
        "bytes": total_bytes,
        "tokens_per_byte": float(tgt_ids.numel() / max(total_bytes, 1)),
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="Validate Scylla data and byte accounting before Mamba-3 training.")
