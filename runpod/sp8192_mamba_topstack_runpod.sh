...
run_train() {
  ...
  MAMBA_ADAPTER_DIM="${MAMBA_ADAPTER_DIM:-48}" \
  ...
  SPARSE_ENABLED="${SPARSE_ENABLED:-0}" \
  SPARSE_TARGET="${SPARSE_TARGET:-0.0}" \
  SPARSE_START_STEP="${SPARSE_START_STEP:-1000}" \
  SPARSE_END_STEP="${SPARSE_END_STEP:-5000}" \
  SPARSE_EVERY="${SPARSE_EVERY:-100}" \
  SPARSE_INCLUDE="${SPARSE_INCLUDE:-mamba_adapter}" \
  SPARSE_MIN_NUMEL="${SPARSE_MIN_NUMEL:-1024}" \
