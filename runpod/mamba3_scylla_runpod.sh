...
install_deps() {
  ...
  python -m pip install -q packaging ninja wheel setuptools einops huggingface-hub sentencepiece tokenmonster datasets tqdm brotli numpy tilelang
  python - <<'PY'
...
run_train() {
  ...
  LOCAL_ATTN_LAYERS="${LOCAL_ATTN_LAYERS:-2}" \
  LOCAL_ATTN_DIM="${LOCAL_ATTN_DIM:-256}" \
  LOCAL_ATTN_HEADS="${LOCAL_ATTN_HEADS:-8}" \
  LOCAL_ATTN_WINDOW="${LOCAL_ATTN_WINDOW:-128}" \
  ...
  TEMP_EVAL_DURING_TRAIN="${TEMP_EVAL_DURING_TRAIN:-0}" \
